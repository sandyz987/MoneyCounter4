                        ...
                        override fun doSomething(respond: String?) {
                            ...
                            activity?.runOnUiThread {
                                talkItem?.let {
                                    ...
                                    includeView?.findViewById<TextView>(R.id.textViewUsrName)?.text =
                                        talkItem.usrName
                                    includeView?.findViewById<TextView>(R.id.textViewTime)?.text =
                                        TimeUtil.getChatTimeStr(talkItem.time)
                                    includeView?.findViewById<TextView>(R.id.textViewContent)?.text =
                                        talkItem.text
                                    ...
                                    val likeAdapter = LikeRecyclerViewAdapter(
                                        activity,
                                        requireContext(),
                                        talkItem.likeAccounts
                                    )
                                    ...
                                    when (talkItem.sex) {
                                        "男" -> includeView?.findViewById<ImageView>(R.id.imageViewSex)
                                            ?.setImageResource(R.drawable.ic_man)
                                        "女" -> includeView?.findViewById<ImageView>(R.id.imageViewSex)
                                            ?.setImageResource(R.drawable.ic_woman)
                                        else -> includeView?.findViewById<ImageView>(R.id.imageViewSex)
                                            ?.setImageBitmap(null)
                                    }
                                    ...
                                    includeView?.findViewById<RoundedImageView>(R.id.imageViewUsrPic)
                                        ?.let {
                                            context?.let { it1 ->
                                                Glide.with(
                                                    it1
                                    ...
                                    if (talkItem.picUrl != null && talkItem.picUrl != "null" && talkItem.picUrl != "") {
                                        includeView?.findViewById<RoundedImageView>(R.id.imageViewPic)
                                            ?.let {
                                                context?.let { it1 ->
                                                    Glide.with(
                                                        it1
                                        ...
                                        includeView?.findViewById<RoundedImageView>(R.id.imageViewPic)?.visibility =
                                            View.VISIBLE
                                    }
                                    includeView?.findViewById<RoundedImageView>(R.id.imageViewUsrPic)
                                        ?.setOnClickListener {
                                    ...
                                    imageViewTalk?.setOnClickListener {
                                        ...
                                        val navController = findNavController()
                                        navController.navigate(
                                            R.id.action_global_fragmentTalkEdit,
                                            bundle
                                        )
                                    }
                                    ...
                                    recyclerViewTalkReplies?.layoutManager =
                                        LinearLayoutManager(context)
                                    if (recyclerViewTalkReplies.itemDecorationCount == 0) {
                                        recyclerViewTalkReplies.addItemDecoration(
                    ...
                    (activity?.application as MainApplication).connectionUrlMain, object :
                        ...
                        override fun doSomething(respond: String?) {
                            activity?.runOnUiThread {
                                when (respond) {
                                    "1" -> {
                                        Toast.makeText(requireContext(), "已删除~", Toast.LENGTH_SHORT)
                                            .show()
                                        val navController = findNavController()
            ...
            if (imageViewLike.getSelect()){
                ...
                HttpUtil.getInstance().httpGet(
                    (activity?.application as MainApplication).connectionUrlMain, object :
                        ...
                        override fun error() {
                            activity?.runOnUiThread {
                                Toast.makeText(activity, "取消点赞失败，请检查网络连接呀", Toast.LENGTH_SHORT)
                                    .show()
                                swipeRefreshLayout?.isRefreshing = false
                            }
