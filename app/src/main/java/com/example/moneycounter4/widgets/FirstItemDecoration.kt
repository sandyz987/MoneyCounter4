...
class FirstItemDecoration(
    ...
    private val isFirst: (pos: Int) -> Boolean,
    private val getText: (pos: Int) -> String
    ...
    override fun onDrawOver(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        parent.getChildAt(0)?.let {
            c.drawText(getText(parent.getChildAdapterPosition(it)), 20F, height - 30F, textPaint)
        }
        ...
        for (i in 1 until parent.childCount) { // 返回显示在屏幕上的数量
            ...
            if (isFirst(parent.getChildAdapterPosition(parent.getChildAt(i)))) {
                parent.getChildAt(0)?.let {
                    c.drawRect(Rect(0, it.top - 100, parent.width, it.top), paint)
                    c.drawText(
                        getText(parent.getChildAdapterPosition(it)),
                        20F,
                        it.top - 30F,
                        textPaint
                    )
